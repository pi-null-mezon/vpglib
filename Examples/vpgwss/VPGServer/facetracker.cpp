...
cv::RotatedRect FaceTracker::searchFace(const cv::Mat &img)
{
    ...
    switch(m_primaryfacedetectortupe) {
        case FaceTracker::ViolaJones:
            ...
            pt_faceClassifier->detectMultiScale(image.getUMat(cv::ACCESS_FAST), rects, 1.15, m_minNeighbours, CV_HAAR_FIND_BIGGEST_OBJECT, m_minFaceSize, m_maxFaceSize );
            break;
...
{    
    ...
    if(faceImg.cols > m_minFaceSize.width && faceImg.rows > m_minFaceSize.height)  {
        ...
        /*cv::Rect2f roiRect(0,0,0,0);
        cv::Point shift(0,0);
        if( (float)img.cols/img.rows > (float)size.width/size.height) {
            roiRect.width = (float)img.cols;
            roiRect.height = img.cols * (float)size.height/size.width;
            shift.y = static_cast<int>((roiRect.height - (float)img.rows)/2.0f);
        } else {
            roiRect.height = (float)img.rows;
            roiRect.width = img.rows * (float)size.width/size.height;
            shift.x = static_cast<int>((roiRect.width - (float)img.cols)/2.0f);
        }
        output = cv::Mat::zeros(roiRect.size(),img.type());
        cv::Mat imgcontent = output(cv::Rect(0,0,img.cols,img.rows)+shift);
        img.copyTo(imgcontent);
        if(roiRect.area() > 0)  {
            int interpolationMethod = CV_INTER_AREA;
            if(size.area() > roiRect.area())
                interpolationMethod = CV_INTER_CUBIC;
...
{
    m_minFaceSize = size;
    m_maxFaceSize = cv::Size(size.width*4, size.height*4);
